...
public class S3Service {
    ...
    @Inject
    S3RequestFactory requestBuilder;

    public Uni<PutObjectResponse> uploadObject(String objectKey, FormData formData) {
        ...
        return Uni.createFrom()
                .completionStage(() -> s3.putObject(request, AsyncRequestBody.fromFile(uploadToTemp(formData.getData()))));
    }

    public Uni<GetResponse> getObject(String objectKey) {
    ...
    public Uni<List<FileObject>> listFiles() {
        ...
        return Uni.createFrom().completionStage(() -> s3.listObjects(request))
                .onItem().transform(this::toFileItems);
    }

    List<FileObject> toFileItems(ListObjectsResponse objects) {
        return objects.contents().stream()
                .sorted(Comparator.comparing(S3Object::lastModified).reversed())
                .map(FileObject::from).collect(Collectors.toList());
    }

    File uploadToTemp(InputStream data) {
        try {
            File tempPath = File.createTempFile("uploadS3Tmp", ".tmp");
    ...
    void startup(
        ...
        s3Client
                ...
                .findFirst()
                .ifPresentOrElse(bucket -> logger.info("Bucket \"{}\" is already present", bucketName), () -> {
